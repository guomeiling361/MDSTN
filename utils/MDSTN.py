...
class MCDAG(nn.Module):
    def __init__(self, input_shape, cross_shape, nb_flows,
                 use_causal_conv=True, temporal_use_transformer=False,
                 use_dense_conv=True, spatial_use_transformer=False,
                 fusion_mode=0):
        super().__init__()
        ...
        if self.use_cross:
            self.cross_embedding = nn.Sequential(
                nn.Conv2d(cross_shape[1], self.feat_dim, 1),
        ...
        # 最终融合层：去掉 meta 后输入通道固定为 feat_dim
    ...
    def forward(self, x, cross_data=None):
        ...
        temporal_input = (
            hist_feat
            .view(batch_size, self.seq_len, -1, self.feat_dim)
            .transpose(2, 1)
            .reshape(-1, self.seq_len, self.feat_dim)
        )

        temporal_feat = (
            self.temporal_branch(temporal_input)[:, -1, :]
            .view(batch_size, self.H, self.W, self.feat_dim)
            .permute(0, 3, 1, 2)
        )
        ...
        if self.use_cross:
            if cross_data is None:
                raise ValueError("模型初始化时 use_cross=True，但 forward 收到的 cross_data=None")

            cross_feat = self.cross_embedding(cross_data)
